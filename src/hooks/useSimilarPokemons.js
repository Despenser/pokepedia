        ...
        const fetchSimilarPokemons = async () => {
            ...
            try {
                ...
                if (cache[cacheKey]) {
                    ...
                    const withNames = await Promise.all(filteredCache.slice(0, 8).map(async (p) => ({
                        ...p,
                        nameRu: await getPokemonNameRu(p.name) || p.name
                    })));
                ...
                const withNames = await Promise.all(filtered.map(async (p) => ({
                    ...p,
                    nameRu: await getPokemonNameRu(p.name) || p.name
                })));
                setSimilarPokemons(withNames);
